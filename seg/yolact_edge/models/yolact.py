...
from models.interpolate import InterpolateModule
from models.warp_utils import deform_op
...
class Yolact(nn.Module):
    ...
    def load_weights2(self, path, args=None):
        """ Loads weights from a compressed save file. """
        state_dict = torch.load(path, map_location='cpu')
        ...
        for key in list(state_dict.keys()):
            ...
            elif key.startswith('fpn.lat_layers'):
                transfered_from_yolact = True
                state_dict[key.replace('fpn.', 'fpn_phase_1.')] = state_dict[key]
                del state_dict[key]
            elif key.startswith('fpn.') and key in state_dict:
